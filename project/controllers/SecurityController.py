...
@app.route('/Security/haveToken', methods = ['POST'])
def haveToken():
	...
	if 'ecoReleve-Core' not in request.cookies:
		abort(make_response('No reneco cookie in request', 401))

	if 'securityKey' not in request.json:
		abort(make_response('No security key provided', 400))

	try:
		decoded = jwt.decode(request.cookies['ecoReleve-Core'], verify=True)
	except:
